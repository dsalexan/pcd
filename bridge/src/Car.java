...
public class Car implements Runnable {
  ...
  private final static int bridgeOuterLeft = 288; // 160
  private final static int bridgeOuterRight = 612; // 340
  ...
  public void move() {
    int xposOld =  xpos;
    if (cartype==REDCAR) {
      if (inFront.getX() - xpos > 100) { // determina se está dentro do escopo ainda
        xpos += 4;
      ...
      if (xpos-inFront.getX() > 100) { // determina se está dentro do escopo ainda
        xpos -= 4;
